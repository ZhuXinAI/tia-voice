  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>{historyTitle || 'Transcription details'}</DialogTitle>
          <DialogDescription>
            {detail ? formatTime(detail.createdAt) : 'Saved audio, raw ASR, and processed output.'}
          </DialogDescription>
        </DialogHeader>

        {loading ? (
          <p className="text-sm text-muted-foreground">Loading debug details…</p>
        ) : detail ? (
          <div className="grid gap-4">
            ...
            <section className="rounded-lg border border-border/70 bg-muted/20 p-4">
              ...
              {detail.audio ? (
                ...
                <p className="mt-3 text-sm text-muted-foreground">No audio clip saved for this item.</p>
              )}
            </section>
            ...
            <section className="grid gap-4 md:grid-cols-2">
              <article className="rounded-lg border border-border/70 bg-muted/20 p-4">
                ...
                <p className="mt-3 whitespace-pre-wrap break-words text-sm leading-6">
                  {detail.transcript || 'No raw transcript captured.'}
                </p>
              </article>
              ...
              <article className="rounded-lg border border-border/70 bg-muted/20 p-4">
                ...
                <p className="mt-3 whitespace-pre-wrap break-words text-sm leading-6">
                  {detail.cleanedText || 'No processed output captured.'}
                </p>
              </article>
            </section>
          ...
          <p className="text-sm text-muted-foreground">This history item no longer has debug details.</p>
        )}
      </DialogContent>
    </Dialog>
  )
