...
import { createMeetingCaptureWindow } from './createMeetingCaptureWindow'

describe('createMeetingCaptureWindow', () => {
  beforeEach(() => {
    browserWindowState.instances.length = 0
    loadRendererWindowMock.mockClear()
  })

  it('creates a clickable always-on-top meeting capture window', async () => {
    await createMeetingCaptureWindow('/tmp/preload.js')

    const window = browserWindowState.instances[0]
    expect(window).toBeDefined()
    expect(window?.options.width).toBe(520)
    expect(window?.options.height).toBe(420)
    ...
    expect(loadRendererWindowMock).toHaveBeenCalledWith(window, 'meeting-capture')
  })
})
